...
function read_endpoint {
    ...
    echo "${name}-mysql-master.${CL_NAMESPACE}"
}
...
function check_job_ok {
    ...
    sleep 10
    kubectl get jobs check-mysql-cluster -o jsonpath="$JSONPATH" | grep "Failed=True"
    if [ $? -eq 0 ]; then
...
function cmd_test {
    ...
    gcs_nodes_upgrade $CLUSTER_NAME $CLUSTER_NEW_VERSION
